...
function getEconomyInterval(): number {
  try {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      const row = db.prepare('SELECT polling_interval_min FROM economy_settings WHERE id = 1').get() as any
      db.close()
...
export function startPolling(port: number): void {
